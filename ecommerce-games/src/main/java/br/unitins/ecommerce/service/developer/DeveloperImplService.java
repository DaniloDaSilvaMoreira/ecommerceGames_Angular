...
public class DeveloperImplService implements DeveloperService {
    ...
    public List<DeveloperResponseDTO> getAll() {
        ...
        return developerRepository.findAll(sort).stream().map(
            developer -> new DeveloperResponseDTO(developer, formatterGetAll, developer.getClassificacao().getLabel())).toList();
    }
